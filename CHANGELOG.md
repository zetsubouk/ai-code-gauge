...
- **`refresh()` 并发守卫**：弹窗消息与定时闹钟同时触发刷新时复用同一次执行，避免重复网络请求与 `lastData`/`history`/`badgeItems` 的竞态写入。
- **弹窗传输层错误不再误标为 GLM**：`doRefresh` 的消息失败路径改用中性 `transport` 来源并渲染进通用状态条；原先只启用 OpenCode Go 时会误显示「GLM 用量查询失败」。
- **徽章循环消除同刻竞争**：`badge-cycle` 闹钟回调改为仅冷启动续醒——SW 存活时 interval 在跑则不动，挂起唤醒（定时器随模块消失）时才恢复并补推一次；原先 interval 与闹钟双驱动在周期交界处会偶发连跳两格。

### Changed
- 弹窗存活期间每 30 秒重绘一次，重置倒计时等相对时间保持跳动（弹窗关闭即销毁，无需清理）。
- 两个面板标题行新增「打开官方面板」跳转按钮（`chrome.tabs.create`，无需新增权限；当前指向平台首页，深层控制台路径确认后可替换）。
- GLM 每周额度文案统一为「每周」（原卡片标题为「本周」，与通知、Go 面板口径不一致）。
